...
class CampTix_Addon_Shortcodes extends CampTix_Addon {
  ...
  function shortcode_attendees( $atts ) {
    ...
    <div id="attendees">
      <div class="row tix-attendee-list tix-columns-<?php echo absint( $columns ); ?>">
        <?php
          while ( true && $printed < $posts_per_page ) {
            ...
            foreach ( $attendees as $attendee_id ) {
              ...
              echo '<div class="col-xs-12 col-lg-4 col-md-4 col-sm-6 tix-attendee-list__item">';

              $first = get_post_meta( $attendee_id, 'tix_last_name', true );
              ...
              if ( file_exists( "http://www.paper-glasses.com/api/twipi/' . $screen_name . '/bigger/" ) )  {
                $twitter_image = "http://www.paper-glasses.com/api/twipi/' . $screen_name . '/bigger/";
              } else {
                $twitter_image = get_avatar_url( get_post_meta( $attendee_id, 'tix_email', true ) );
              }
              ...
              if ( $screen_name ) {
                ...
                echo '<div class="col-xs-4"><img src="' . get_avatar_url( get_post_meta( $attendee_id, 'tix_email', true ) ) . '" class="img-responsive img-circle"  width="50" height="50" /></div>';
              }
      ...
      <form method="POST" action="<?php add_query_arg( null, null ); ?>#tix">
        ...
        <table class="tix-private-form">
          ...
          <tr>
            ...
            <td class="tix-right"><input name="tix_first_name" value="<?php echo esc_attr( $first_name ); ?>" type="text" class="form-control" /></td>
          </tr>
          <tr>
            <td class="tix-left"><?php _e( 'Last Name', 'camptix' ); ?></td>
            <td class="tix-right"><input name="tix_last_name" value="<?php echo esc_attr( $last_name ); ?>" type="text" class="form-control" /></td>
          </tr>
          <tr>
            <td class="tix-left"><?php _e( 'E-mail', 'camptix' ); ?></td>
            <td class="tix-right"><input name="tix_email" value="<?php echo esc_attr( $email ); ?>" type="text" class="form-control" /></td>
          </tr>
        </table>
        <p class="tix-submit">
          <input type="submit" value="<?php esc_attr_e( 'Login &rarr;', 'camptix' ); ?>" class="btn btn-default">
          <br class="tix-clear">
        </p>
      </form>
    </div>
