...
function N2ZipRead($file) {
    if (!function_exists('zip_open')) {
        N2Message::error(n2_('The zip_open function is missing from your server. ZipArchive has to be enabled on your server to use this functionality! Ask your server host to enable it.'));
        return null;
    } else {
        $zip = zip_open($file);
        if (!is_resource($zip)) {
