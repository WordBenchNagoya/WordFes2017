...
class CampTix_Plugin {
  ...
  function __construct() {
    ...
    require( dirname( __FILE__ ) . '/inc/class-camptix-widget.php' );
    ...
    add_action( 'publish_tix_attendee', array( $this, 'publish_tix_attendee' ), 10, 2 );
    // Load a text domain
  ...
  function init() {
    ...
    add_action( 'load-tix_ticket_page_camptix_tools', array(
      $this,
      'summarize_admin_init'
    ) ); // marked as admin init but not really
  ...
  function admin_enqueue_scripts() {
    ...
    if ( is_admin() ) {
      ...
      if (
        ...
        wp_enqueue_script( 'camptix-admin', plugins_url( '/admin.js', __FILE__ ), array(
          'jquery',
  ...
  function custom_columns() {
    ...
    add_action( 'manage_tix_attendee_posts_custom_column', array(
      $this,
      'manage_columns_attendee_action'
    ), 10, 2 );
  ...
  function manage_columns_attendee_filter( $columns ) {
    ...
    $columns['tix_name']   = __( '振込名義人', 'camptix' );
  ...
  function update_hidden_columns() {
    if ( ! empty( $_REQUEST['post_type'] ) && ! in_array( $_REQUEST['post_type'], array(
        'tix_attendee',
        'tix_ticket'
      ) )
    ) {
  ...
  function menu_setup_controls() {
    ...
    switch ( $section ) {
      case 'general':
        add_settings_section( 'general', __( 'General Configuration', 'camptix' ), array(
          $this,
          'menu_setup_section_general'
        ), 'camptix_options' );
      ...
      case 'payment':
        foreach ( $this->get_available_payment_methods() as $key => $payment_method ) {
          ...
          add_settings_section( 'payment_' . $key, $payment_method_obj->name, array(
            $payment_method_obj,
            '_camptix_settings_section_callback'
          ), 'camptix_options' );
          add_settings_field( 'payment_method_' . $key . '_enabled', __( 'Enabled', 'camptix' ), array(
            $payment_method_obj,
      ...
      case 'email-templates':
        add_settings_section( 'general', __( 'E-mail Templates', 'camptix' ), array(
          $this,
          'menu_setup_section_email_templates'
        ), 'camptix_options' );
      ...
      case 'beta':
        ...
        add_settings_section( 'general', __( 'Beta Features', 'camptix' ), array(
          $this,
          'menu_setup_section_beta'
        ), 'camptix_options' );
  ...
  function field_checkbox( $args ) {
    ...
    <input type="checkbox" id="<?php echo esc_attr( $args['name'] ); ?>" name="<?php echo esc_attr( $args['name'] ); ?>" class="<?php echo sanitize_html_class( $args['class'] ); ?>" value="1"
  ...
  function field_yesno( $args ) {
    ?>
    <label class="tix-yes-no description"><input type="radio" name="<?php echo esc_attr( $args['name'] ); ?>" value="1" <?php checked( $args['value'], true ); ?>> <?php _e( 'Yes', 'camptix' ); ?>
    </label>
    <label class="tix-yes-no description"><input type="radio" name="<?php echo esc_attr( $args['name'] ); ?>" value="0" <?php checked( $args['value'], false ); ?>> <?php _e( 'No', 'camptix' ); ?>
    </label>
  ...
  function field_enable_refunds( $args ) {
    ...
    <div id="tix-refunds-date" class="<?php if ( ! $refunds_enabled ) {
      echo 'hide-if-js';
    } ?>" style="margin: 20px 0;">
  ...
  function get_currencies() {
    return apply_filters( 'camptix_currencies', array(
      ...
      'TRY' => array(
        ...
        'format' => 'TRY %s',
        // Unicode is &#8378; but this doesn't seem to be widely supported yet (introduced Sep 2012)
      ),
    ) );
  }
  ...
  function append_currency( $price, $nbsp = true, $currency_key = false ) {
    ...
    $with_currency = sprintf( $currency['format'], number_format( (float) $price ) );
    if ( $nbsp ) {
   ...
   */
  function admin_menu() {
    add_submenu_page( 'edit.php?post_type=tix_ticket', __( 'Tools', 'camptix' ), __( 'Tools', 'camptix' ), $this->caps['manage_tools'], 'camptix_tools', array(
      $this,
      'menu_tools'
    ) );
    add_submenu_page( 'edit.php?post_type=tix_ticket', __( 'Setup', 'camptix' ), __( 'Setup', 'camptix' ), $this->caps['manage_options'], 'camptix_options', array(
      $this,
      'menu_setup'
    ) );
          ...
          <tr>
            ...
            <td>
              <select name="tix_export_to">
                ...
                <option disabled="disabled" value="pdf">
                  PDF <?php _e( '(coming soon)', 'camptix' ); ?></option>
              </select>
            </td>
          </tr>
        </tbody>
          ...
          <tr>
            ...
            <td>
              ...
              <?php while ( $tickets_query->have_posts() ) : $tickets_query->the_post(); ?>
                <label><input type="checkbox" <?php checked( array_key_exists( get_the_ID(), $form_data['tickets'] ) ); ?> name="tix_notify_tickets[<?php the_ID(); ?>]" value="1"/> <?php echo sanitize_text_field( get_the_title() ); ?>
                </label><br/>
              <?php endwhile; ?>
            </td>
          </tr>
              ...
              <?php if ( ! empty( $shortcode_tags ) ) : ?>
                <p class=""><?php _e( 'You can use the following shortcodes:', 'camptix' ); ?>
                  <?php foreach ( $shortcode_tags as $key => $tag ) : ?>
                    <a href="#" class="tix-notify-shortcode"><code>[<?php echo esc_html( $key ); ?>
                                                                   ]</code></a>
                  <?php endforeach; ?>
                </p>
              <?php endif; ?>
      ...
      <table class="form-table">
        <tbody>
          <tr>
            ...
            <td>
              <label><input name="tix_refund_checkbox_1" value="1" type="checkbox"/> <?php _e( 'Refund all transactions', 'camptix' ); ?>
              </label><br/>
              <label><input name="tix_refund_checkbox_2" value="1" type="checkbox"/> <?php _e( 'Seriously, refund them all', 'camptix' ); ?>
              </label><br/>
              <label><input name="tix_refund_checkbox_3" value="1" type="checkbox"/> <?php _e( "I know what I'm doing, please refund", 'camptix' ); ?>
              </label><br/>
              <label><input name="tix_refund_checkbox_4" value="1" type="checkbox"/> <?php _e( 'I know this may result in money loss, refund anyway', 'camptix' ); ?>
              </label><br/>
              <label><input name="tix_refund_checkbox_5" value="1" type="checkbox"/> <?php _e( 'I will not blame Konstantin if something goes wrong', 'camptix' ); ?>
              </label><br/>
            </td>
          </tr>
        </tbody>
  ...
  function add_meta_boxes() {
    add_meta_box( 'tix_ticket_options', __( 'Ticket Options', 'camptix' ), array(
      $this,
      'metabox_ticket_options'
    ), 'tix_ticket', 'side' );
    add_meta_box( 'tix_ticket_availability', __( 'Availability', 'camptix' ), array(
      $this,
      'metabox_ticket_availability'
    ), 'tix_ticket', 'side' );
    add_meta_box( 'tix_ticket_questions', __( 'Questions', 'camptix' ), array(
      $this,
      'metabox_ticket_questions'
    ), 'tix_ticket' );

    if ( $this->options['reservations_enabled'] ) {
      add_meta_box( 'tix_ticket_reservations', __( 'Reservations', 'camptix' ), array(
        $this,
        'metabox_ticket_reservations'
      ), 'tix_ticket' );
    }

    add_meta_box( 'tix_coupon_options', __( 'Coupon Options', 'camptix' ), array(
      $this,
      'metabox_coupon_options'
    ), 'tix_coupon', 'side' );
    add_meta_box( 'tix_coupon_availability', __( 'Availability', 'camptix' ), array(
      $this,
      'metabox_coupon_availability'
    ), 'tix_coupon', 'side' );

    add_meta_box( 'tix_attendee_info', __( 'Attendee Information', 'camptix' ), array(
      $this,
      'metabox_attendee_info'
    ), 'tix_attendee', 'normal' );

    add_meta_box( 'tix_attendee_submitdiv', __( 'Publish', 'camptix' ), array(
      $this,
      'metabox_attendee_submitdiv'
    ), 'tix_attendee', 'side' );
    ...
    <div class="submitbox" id="submitpost">
      ...
      <div id="major-publishing-actions">
        ...
        <div id="publishing-action">
          <?php submit_button( __( 'Save Attendee', 'camptix' ), 'primary', 'save', false, array(
            'tabindex'  => '5',
            'accesskey' => 'p'
          ) ); ?>
        </div>
  ...
  function metabox_ticket_options() {
    ...
    <div class="misc-pub-section">
      ...
      <?php else: ?>
        <span><?php echo esc_html( $this->append_currency( get_post_meta( get_the_ID(), 'tix_price', true ) ) ); ?></span>
        <br/>
        ...
        <table>
          ...
          <tbody>
            <?php foreach ( $reservations as $reservation ) : ?>
              <tr>
                <td>
                  <span><?php echo esc_html( isset( $reservation['name'] ) ? $reservation['name'] : urldecode( $reservation['id'] ) ); ?></span>
                </td>
                <td class="column-quantity">
                  <span><?php echo intval( $reservation['quantity'] ); ?></span></td>
                <td class="column-used">
                  <span><?php echo $this->get_purchased_tickets_count( get_the_ID(), $reservation['token'] ); ?></span>
                </td>
                <td class="column-token"><span><a href="<?php echo esc_url( $this->get_reservation_link( $reservation['id'], $reservation['token'] ) ); ?>"><?php echo $reservation['token']; ?></a></span>
                </td>
  ...
  function question_field_checkbox( $name, $user_value, $question ) {
    ...
    <?php if ( $values ) : ?>
      <?php foreach ( (array) $values as $question_value ) : ?>
        <label><input <?php checked( in_array( $question_value, (array) $user_value ) ); ?> name="<?php echo esc_attr( $name ); ?>[<?php echo sanitize_title_with_dashes( $question_value ); ?>]" type="checkbox" value="<?php echo esc_attr( $question_value ); ?>"/> <?php echo esc_html( $question_value ); ?>
        </label><br/>
      <?php endforeach; ?>
    <?php else : ?>
      <label><input <?php checked( $user_value, 'Yes' ); ?> name="<?php echo esc_attr( $name ); ?>" type="checkbox" value="Yes"/> <?php _e( 'Yes', 'camptix' ); ?>
      </label>
    <?php endif; ?>
  <?php
  }
  ...
  function question_field_radio( $name, $user_value, $question ) {
    ...
    ?>
    <?php foreach ( (array) $values as $question_value ) : ?>
      <label><input <?php checked( $question_value, $user_value ); ?> name="<?php echo esc_attr( $name ); ?>" type="radio" value="<?php echo esc_attr( $question_value ); ?>"/> <?php echo esc_html( $question_value ); ?>
      </label><br/>
    <?php endforeach; ?>
  <?php
  }
      ...
      <script type="text/template" id="camptix-tmpl-new-question-form">
        ...
        <table class="form-table">
          ...
          <tr valign="top">
            ...
            <td>
              <label><input data-model-attribute="required" data-model-attribute-type="checkbox" id="tix-add-question-required" type="checkbox" value="1"/> <?php _e( 'This field is required', 'camptix' ); ?>
              </label>
            </td>
          </tr>
        </table>
  ...
  function metabox_coupon_options() {
    ...
    <div class="misc-pub-section">
      ...
      <?php if ( $used <= 0 ) : ?>
        <input type="text" name="tix_discount_price" class="small-text" style="width: 57px;" value="<?php echo esc_attr( $discount_price ); ?>" autocomplete="off"/> <?php echo esc_html( $this->options['currency'] ); ?>
        <br/>
    ...
    <div class="misc-pub-section tix-applies-to">
      ...
      <div class="tix-checkbox-group">
        <label style="margin-bottom: 8px;">
          <a id="tix-applies-to-all" href="#"><?php _e( 'All', 'camptix' ); ?></a>
          /
          <a id="tix-applies-to-none" href="#"><?php _e( 'None', 'camptix' ); ?></a>
        </label>
        <?php
        ...
        <?php while ( $q->have_posts() ) : $q->the_post(); ?>
          <label><input <?php checked( in_array( $post->ID, $applies_to ) ); ?> type="checkbox" class="tix-applies-to-checkbox" name="tix_applies_to[]" value="<?php the_ID(); ?>"/> <?php echo sanitize_text_field( get_the_title() ); ?>
          </label>
        <?php endwhile; ?>
  ...
  function metabox_attendee_info() {
    ...
    $rows[] = array(
      __( 'First Name', 'camptix' ),
      esc_html( get_post_meta( $post->ID, 'tix_first_name', true ) )
    );
    ...
    $rows[] = array(
      __( 'Ticket', 'camptix' ),
      sprintf( '<a href="%s">%s</a>', get_edit_post_link( $ticket->ID ), $ticket->post_title )
    );

    $rows = apply_filters( 'camptix_metabox_attendee_info_additional_rows', $rows, $post );

    $rows[] = array(
      __( 'Edit Token', 'camptix' ),
      sprintf( '<a href="%s">%s</a>', $this->get_edit_attendee_link( $post->ID, $edit_token ), $edit_token )
    );
    $rows[] = array(
      __( 'Access Token', 'camptix' ),
      sprintf( '<a href="%s">%s</a>', $this->get_access_tickets_link( $access_token ), $access_token )
    );
    ...
    if ( $coupon_id ) {
      ...
      $rows[] = array(
        __( 'Coupon', 'camptix' ),
        sprintf( '<a href="%s">%s</a>', get_edit_post_link( $coupon->ID ), $coupon->post_title )
      );
    }

    $rows[] = array(
      __( 'Order Total', 'camptix' ),
      $this->append_currency( get_post_meta( $post->ID, 'tix_order_total', true ) )
    );
    ...
    if ( $this->options['reservations_enabled'] ) {
      ...
      if ( $reservation_id && $reservation_token ) {
        $rows[] = array(
          __( 'Reservation', 'camptix' ),
          sprintf( '<a href="%s">%s</a>', esc_url( $reservation_url ), esc_html( $reservation_id ) )
        );
      }
    }
  ...
  function form_start() {
    ...
    <div id="tix">
      ...
      <?php if ( $available_tickets ) : ?>
        <form id="tix-entry-form" action="<?php echo esc_url( add_query_arg( 'tix_action', 'attendee_info', $this->get_tickets_url() ) ); ?>#tix" method="POST">
          ...
          <table class="tix_tickets_table">
            <thead>
              <tr>
                <th class="tix-column-quantity" width="10%">
                  参加
                </th>
                <th class="tix-column-description">チケットの種類</th>
            ...
            <tbody>
              <?php foreach ( $this->tickets as $ticket ) : ?>
                ...
                <tr class="tix-ticket-<?php echo absint( $ticket->ID ); ?>">
                  <td class="tix-column-quantity text-center" style="vertical-align: middle;">
                    <input class="ticket-quantity" type="checkbox" name="tix_tickets_selected[<?php echo $ticket->ID; ?>]" value="1" <?php if ( $ticket_key == 370 ) {
                      echo 'checked';
                    } ?>>
                    <label></label>
                    <?php
                  ...
                  <td class="tix-column-description">
                    ...
                    <?php if ( $ticket->post_excerpt ) : ?>
                      <br/>
                      <span class="tix-ticket-excerpt"><?php echo $ticket->post_excerpt; ?></span>
                    <?php endif; ?>
                    <?php if ( $ticket->tix_coupon_applied ) : ?>
                      <br/>
                      <small class="tix-discount"><?php echo esc_html( $ticket->tix_discounted_text ); ?></small>
                    <?php endif; ?>
                  </td>
                    <?php if ( $price > 0 ) : ?>
                  <td class="tix-column-price text-right" style="vertical-align: middle;">
                      <?php echo $this->append_currency( $price ); ?>
                  </td>
                    <?php else : ?>
                  <td class="tix-column-price text-center" style="vertical-align: middle;">
                      無料
                  </td>
                    <?php endif; ?>
                  <?php if ( apply_filters( 'camptix_show_remaining_tickets', true ) ) : ?>
                    <td class="tix-column-remaining text-right" style="vertical-align: middle;"><?php echo apply_filters( 'camptix_form_start_tix_remaining', $ticket->tix_remaining, $ticket ); ?></td>
                  <?php endif; ?>
                  <?php /*
          ...
          <div class="text-center">
          	...
          	<p id="error-message" class="text-center" style="padding: 20px 0 10px; color: red; font-weight: bold;">&nbsp;</p>

            <?php /* <input type="submit" value="<?php esc_attr_e( 'Register &rarr;', 'camptix' ); ?>" style="float: right; cursor: pointer;" class="<?php echo implode( ' ', apply_filters( 'camptix_register_button_classes', array() ) ); ?>"/> */ ?>
            <input id="tix-form-submit" type="submit" value="チケットを選択" style="cursor: pointer;" class="<?php echo implode( ' ', apply_filters( 'camptix_register_button_classes', array() ) ); ?>"/>
            <br class="tix-clear"/>
          </div>
        </form>
      <?php endif; ?>
      ...
      <form action="<?php echo esc_url( add_query_arg( 'tix_action', 'checkout' ), $this->get_tickets_url() ); ?>#tix" method="POST">
            ...
            <?php foreach ( $this->tickets_selected as $ticket_id => $count ) : ?>
              ...
              <tr>
                <td class="tix-column-description">
                  ...
                  <?php if ( $ticket->tix_coupon_applied ) : ?>
                    <br/>
                    <small><?php echo $ticket->tix_discounted_text; ?></small>
                  <?php endif; ?>
                </td>
                  <?php if ( $price > 0 ) : ?>
                <td class="tix-column-per-ticket text-right">
                    <?php echo $this->append_currency( $price ); ?>
                </td>
                  <?php else : ?>
                <td class="tix-column-per-ticket text-center">
                    無料
                </td>
                  <?php endif; ?>
            ...
            <table class="tix_tickets_table tix-attendee-form">
              <tbody>
                ...
                <tr class="tix-row-last-name">
                  <td class="tix-required tix-left"><?php _e( 'Last Name', 'camptix' ); ?>
                ...
                <tr class="tix-row-first-name">
                  <td class="tix-required tix-left"><?php _e( 'First Name', 'camptix' ); ?>
                    <span class="tix-required-star">*</span></td>
                ...
                <tr class="tix-row-email">
                  <td class="tix-required tix-left"><?php _e( 'E-mail', 'camptix' ); ?>
                    <span class="tix-required-star">*</span></td>
                  ...
                  <td class="tix-right">
                    ...
                    <?php if ( $this->tickets_selected_count > 1 ) : ?>
                      <div class="tix-hide-if-js">
                        <label><input name="tix_receipt_email" <?php checked( $tix_receipt_email, $i ); ?> value="<?php echo $i; ?>" type="radio"/> <?php _e( 'Send the receipt to this address', 'camptix' ); ?>
                        </label>
                      </div>
                    <?php else: ?>
        ...
        <?php if ( $this->tickets_selected_count > 1 ) : ?>
          <div class="tix-show-if-js">
            <table class="tix-receipt-form">
              ...
              <tr>
                <td class="tix-left tix-required"><?php _e( 'E-mail the receipt to', 'camptix' ); ?>
                  <span class="tix-required-star">*</span></td>
                <td class="tix-right" id="tix-receipt-emails-list">
                  <?php if ( isset( $this->form_data['tix_receipt_email_js'] ) && is_email( $this->form_data['tix_receipt_email_js'] ) ) : ?>
                    <label><input name="tix_receipt_email_js" checked="checked" value="<?php echo esc_attr( $this->form_data['tix_receipt_email_js'] ); ?>" type="radio"/> <?php echo esc_html( $this->form_data['tix_receipt_email_js'] ); ?>
                    </label><br/>
                  <?php endif; ?>
                </td>
              </tr>
        ...
        <p class="tix-submit">
          <?php if ( $total > 0 ) : ?>
            ...
            <?php /* <input type="submit" value="<?php esc_attr_e( 'Checkout &rarr;', 'camptix' ); ?>"/> */ ?>
            <input type="submit" value="<?php echo esc_attr('参加登録'); ?>"/>
          <?php else : ?>
            <?php /* <input type="submit" value="<?php esc_attr_e( 'Claim Tickets &rarr;', 'camptix' ); ?>"/> */ ?>
            <input type="submit" value="<?php echo esc_attr('参加登録'); ?>"/>
          <?php endif; ?>
          <br class="tix-clear"/>
        </p>
  ...
  function form_refund_request() {
    global $post, $this_year, $email_addr;
    ...
    foreach ( $attendees as $attendee ) {
      ...
      // if ( $txn_id ) {
      $transactions = get_post_meta( $attendee->ID, 'tix_transaction_details', true );
      // $transactions[ $txn_id ]['transaction_id'] = $txn_id;
      $transactions['payment_amount'] = get_post_meta( $attendee->ID, 'tix_order_total', true );
      $transactions['receipt_email']  = get_post_meta( $attendee->ID, 'tix_receipt_email', true );
      $transactions['payment_method'] = get_post_meta( $attendee->ID, 'tix_payment_method', true );
      $transactions['payment_token']  = get_post_meta( $attendee->ID, 'tix_payment_token', true );
      // }
      $ticket_id = get_post_meta( $attendee->ID, 'tix_ticket_id', true );
    ...
    if ( isset( $_POST['tix_refund_request_submit'] ) ) {
      ...
      // 追加
      $transfer         = esc_html( $_POST['tix_refund_request_transfer'] );
      $transfer_method  = esc_html( $_POST['tix_refund_request_transfer_method'] );
      $bank_name        = esc_html( $_POST['tix_refund_bank_name'] );
      $bank_branch_name = esc_attr( $_POST['tix_refund_bank_branch_name'] );
      $bank_port_type   = esc_attr( $_POST['tix_refund_bank_port_type'] );
      ...
      if ( ! $check ) {
        ...
        foreach ( $attendees as $attendee ) {
          ...
          $cancel_info =
            <<< INFO
%s 様
WordFes Nagoya %s のチケットキャンセルが正常に完了しました。
...
INFO;
           ...
           * @var [type]
           */
          $content = sprintf( $cancel_info, $attendee->post_title, $this_year, $transactions['receipt_email'], $transaction['payment_amount'], $transfer_str, $bank_port_type_str, $bank_name, $bank_branch_name, $transfer_method_str, $bank_port_name, $bank_port_number, $reason, $this_year );
    ...
    <div id="tix">
      ...
      <form action="<?php echo esc_url( add_query_arg( 'tix_action', 'refund_request' ) ); ?>#tix" method="POST">
        ...
        <h2 class="sub-title02"><?php _e( 'Refund Request', 'camptix' ); ?></h2>
        <table class="tix_tickets_table table table-bordered  tix-attendee-form">
          <tbody>
            <tr>
              <th colspan="2">
            ...
            <tr>
              ...
              <td class="tix-right"><?php printf( "%s %s", $this->options['currency'], number_format( $transaction['payment_amount'] ) ); ?></td>
            </tr>
            <tr>
              <td class="tix-left"><?php _e( 'Purchased Tickets', 'camptix' ); ?></td>
              <td class="tix-right">
                <?php foreach ( $tickets as $ticket_id => $count ) : ?>
                  <?php echo esc_html( sprintf( "%s x%d", $this->get_ticket_title( $ticket_id ), $count ) ); ?>
                  <br/>
                <?php endforeach; ?>
              </td>
            </tr>
            ...
            <tr>
              <td class="tix-left"><?php _e( '振込', 'camptix' ); ?></td>
              <td class="tix-right">
                <label for="tix_refund_request_transfer_true">
                  <input id="tix_refund_request_transfer_true" type="radio" name="tix_refund_request_transfer" value="1">
                  済
                </label>
                <label for="tix_refund_request_transfer_false">
                  <input id="tix_refund_request_transfer_false" type="radio" name="tix_refund_request_transfer" value="0">
                  未
                </label>
              </td>
            </tr>
            <tr>
              <td class="tix-left"><?php _e( '入金済みの方は返金の方法をお選びください', 'camptix' ); ?></td>
              <td class="tix-right">
                <label for="tix_refund_request_transfer_method_true">
                  <input id="tix_refund_request_transfer_method_true" type="radio" name="tix_refund_request_transfer_method" value="1">
                  当日受付にて現金で返金
                </label>
                <label for="tix_refund_request_transfer_method_false">
                  <input id="tix_refund_request_transfer_method_false" type="radio" name="tix_refund_request_transfer_method" value="0">
                  振込で返金（振込手数料を除いた金額を返金）
                </label>
              </td>
            </tr>
            <tr>
              <th colspan="2">振込を選ばれた方は、振込先をご入力ください</th>
            </tr>
            <tr>
              <td class="tix-left"><?php _e( '金融機関名', 'camptix' ); ?></td>
              <td class="tix-right">
                <input type="text" class="form-control" name="tix_refund_bank_name" value="">
              </td>
            </tr>
            <tr>
              <td class="tix-left"><?php _e( '支店名', 'camptix' ); ?></td>
              <td class="tix-right">
                <input type="text" class="form-control" name="tix_refund_bank_branch_name" value="">
              </td>
            </tr>
            <tr>
              <td class="tix-left"><?php _e( '口座種類', 'camptix' ); ?></td>
              <td class="tix-right">
                <label for="tix_refund_bank_port_type_true">
                  <input type="radio" id="tix_refund_bank_port_type_true" name="tix_refund_bank_port_type" value="1">
                  普通
                </label>
                <label for="tix_refund_bank_port_type_false">
                  <input type="radio" id="tix_refund_bank_port_type_false" name="tix_refund_bank_port_type" value="0">
                  当座
                </label>
              </td>
            </tr>
            <tr>
              <td class="tix-left"><?php _e( '口座名義', 'camptix' ); ?></td>
              <td class="tix-right">
                <input type="text" class="form-control" name="tix_refund_bank_port_name" value="">
              </td>
            </tr>
            <tr>
              <td class="tix-left"><?php _e( '口座番号', 'camptix' ); ?></td>
              <td class="tix-right">
                <input type="text" class="form-control" name="tix_refund_bank_port_number" value="">
              </td>
            </tr>
            <tr>
              <td class="tix-left"><?php _e( 'Refund Reason', 'camptix' ); ?></td>
              <td class="tix-right">
                <textarea class="form-control" name="tix_refund_request_reason"><?php echo esc_textarea( $reason ); ?></textarea>
              </td>
            </tr>
        ...
        <p class="tix-submit">
          <label><input type="checkbox" name="tix_refund_request_confirmed" value="1"> <?php _e( 'I agree to the above terms', 'camptix' ); ?>
          </label>
          <input type="submit" class="btn btn-default" value="<?php esc_attr_e( 'Send Request', 'camptix' ); ?>"/>
          <br class="tix-clear"/>
        </p>
      </form>
    </div><!-- #tix -->
  ...
  function form_refund_success() {
    ...
    ?>
    <div id="tix">
      <h2 class="sub-title02"><?php _e( 'キャンセル完了', 'camptix' ); ?></h2>
  ...
  function is_ticket_valid_for_purchase( $post ) {
    ...
    if ( apply_filters( 'camptix_hide_empty_tickets', true ) && $this->get_remaining_tickets( $post->ID, $via_reservation ) < 1 ) {
      return false;
    }
  ...
  function is_coupon_valid_for_use( $coupon_id ) {
    $coupon = get_post( $coupon_id );
    if ( $coupon->post_type != 'tix_coupon' ) {
      return false;
    }
    if ( $coupon->post_status != 'publish' ) {
      return false;
    }
    if ( $this->get_remaining_coupons( $coupon->ID ) < 1 ) {
      return false;
    }
  ...
  function email_tickets( $payment_token = false, $from_status = 'draft', $to_status = 'publish' ) {
     ...
     */
    if ( count( $attendees ) > 1 && $from_status == 'draft' && ( in_array( $to_status, array(
        'publish',
        'pending'
      ) ) )
    ) {
      foreach ( $attendees as $attendee ) {
        ...
        $subject = sprintf( __( "Your Ticket to %s", 'camptix' ), $this->options['event_name'] ) . '申し込み完了のお知らせ';
    ...
    if ( count( $attendees ) > 1 && 'publish' == $from_status && 'refund' == $to_status ) {
      ...
      foreach ( $attendees as $attendee ) {
        ...
        if ( $attendee_email != $receipt_email ) {
          $subject        = sprintf( __( "Your Refund for %s", 'camptix' ), $this->options['event_name'] ) . ' キャンセル完了のお知らせ';
        ...
        <?php foreach ( $rows as $row ) : ?>
          ...
          <tr class="<?php echo $alt; ?> tix-row-<?php echo sanitize_title_with_dashes( array_shift( $values ) ); ?>">
            <?php foreach ( $row as $column => $value ) : ?>
              <td class="tix-<?php echo sanitize_title_with_dashes( $column ); ?>">
                <span><?php echo $value; ?></span></td>
            <?php endforeach; ?>
          </tr>
        <?php endforeach; ?>
        <?php
        // 追加
        $reason = get_post_meta( $_GET['post'], 'tix_refund_reason', true );
        if ( $reason = get_post_meta( $_GET['post'], 'tix_refund_reason', true ) && $reason ) { ?>
          <tr>
            <th colspan="2" style="height: 30px;text-align: left;padding-left: 10px;background: #ccc;">
              キャンセル
            </th>
          </tr>
          <tr class="tix-row alternate">
            <td class="tix-0">
              <span>振込</span>
            </td>
            <td class="tix-1">
              <?php if ( ( $refund_transfer = get_post_meta( $_GET['post'], 'tix_refund_transfer', true ) ) && $refund_transfer === '1' ) {
                echo '済';
              } else {
                echo '未';
              }; ?>
          ...
          <tr class="tix-row ">
            ...
            <td class="tix-1">
              <?php if ( ( $transfer_method = get_post_meta( $_GET['post'], 'tix_refund_request_transfer_method', true ) ) && $transfer_method === '1' ) {
                echo '当日受付にて現金で返金';
              } else {
                echo '振込で返金';
              }; ?>
          ...
          <tr class="alternate">
            ...
            <td>
              <?php
              $tix_refund_bank_name = ( get_post_meta( $_GET['post'], 'tix_refund_bank_name', true ) ) ? get_post_meta( $_GET['post'], 'tix_refund_bank_name', true ) : '';
              echo esc_html( $tix_refund_bank_name );
              ?>
            </td>
          </tr>
          <tr class="">
            ...
            <td>
              <?php
              $tix_refund_bank_branch_name = ( get_post_meta( $_GET['post'], 'tix_refund_bank_branch_name', true ) ) ? get_post_meta( $_GET['post'], 'tix_refund_bank_branch_name', true ) : '';
              echo esc_html( $tix_refund_bank_branch_name );
              ?>
            </td>
          </tr>
          <tr class="alternate">
            ...
            <td>
              <?php if ( ( $tix_refund_bank_port_type = get_post_meta( $_GET['post'], 'tix_refund_bank_port_type', true ) ) && $tix_refund_bank_port_type === '1' ) {
                echo '普通';
              } else {
                echo '当座';
              }; ?>
          ...
          <tr class="">
            ...
            <td>
              <?php
              $tix_refund_bank_port_name = ( get_post_meta( $_GET['post'], 'tix_refund_bank_port_name', true ) ) ? get_post_meta( $_GET['post'], 'tix_refund_bank_port_name', true ) : '';
              echo esc_html( $tix_refund_bank_port_name );
              ?>
            </td>
          </tr>
          <tr class="alternate">
            ...
            <td>
              <?php
              $tix_refund_bank_port_number = ( get_post_meta( $_GET['post'], 'tix_refund_bank_port_number', true ) ) ? get_post_meta( $_GET['post'], 'tix_refund_bank_port_number', true ) : '';
              echo esc_html( $tix_refund_bank_port_number );
              ?>
            </td>
          </tr>
          <tr class="">
            ...
            <td>
              <?php
              $tix_refund_reason = ( get_post_meta( $_GET['post'], 'tix_refund_reason', true ) ) ? get_post_meta( $_GET['post'], 'tix_refund_reason', true ) : '';
              echo esc_html( $tix_refund_reason );
              ?>
            </td>
          </tr>
  ...
  public function publish_tix_attendee( $ID, $post ) {
    ...
    $transactions                   = array();
    $transactions['payment_amount'] = get_post_meta( $post->ID, 'tix_order_total', true );
    $transactions['receipt_email']  = get_post_meta( $post->ID, 'tix_receipt_email', true );
    ...
    $admin_email                    = $email_addr;
    $subject                        = "振込確認完了のお知らせ | WordFes Nagoya $this_year ";
    $name = get_post_meta( $post->ID, 'tix_last_name', true ) . get_post_meta( $post->ID, 'tix_first_name', true );
    $main_info                      =
      <<< INFO
%s 様
WordFes Nagoya %s への銀行振込を確認しました。
http://%s.wordfes.org/entry/
...
INFO;
    ...
    $content = sprintf( $main_info, $name, $this_year, $this_year, $this_year );

    $this->wp_mail( $transactions['receipt_email'], $subject, $content );
    // To entry@wordfes.org
    $this->wp_mail( $admin_email, $subject, $content );
        $this->log( sprintf( '%sへ振込確認のメールを送信しました。', $transactions['receipt_email'] ), $post->ID );
    /**
     * End cancel meta to user post
     */
    // $success = update_post_meta( $post->ID, 'post_status',  );
    // $this->log( 'Refund reason attached with data.', $post->ID, $reason, 'refund' );
  }
